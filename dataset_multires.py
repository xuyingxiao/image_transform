...
# from projects.mapalign.utils import visualization
...
def create_dataset_filename_list(tfrecords_dir_list, tfrecord_filename_format, dataset="train",
    ...
    for tfrecords_dir_index,tfrecords_dir in enumerate(tfrecords_dir_list):
        print("tfrecords_dir",tfrecords_dir,tfrecords_dir_index)
        for resolution_file_repeat in resolution_file_repeats:
            for dir in os.listdir(os.path.join(tfrecords_dir,dataset)):
                shard_filepath_format = os.path.join(tfrecords_dir,tfrecord_filename_format.format(dataset,dir))
...
def read_and_decode(tfrecord_filepaths, patch_inner_res, patch_outer_res, batch_size,
    ...
    with tf.name_scope('read_and_decode'):
        ...
        if train:
            features = tf.parse_single_example(
                ...
                features={
                    ...
                    'rotate_theta': tf.FixedLenFeature([], tf.float32),
                    'scale_x': tf.FixedLenFeature([], tf.float32),
                    'scale_y': tf.FixedLenFeature([], tf.float32),
                    'offset_x': tf.FixedLenFeature([], tf.float32),
                    'offset_y': tf.FixedLenFeature([], tf.float32),
        ...
        disp_field_map_flat = tf.decode_raw(features['disp_field_maps'], tf.int16)
        disp_polygon_map_flat = tf.decode_raw(features['disp_polygon_maps'], tf.uint8)
        rotate_theta_flat = features['rotate_theta']
        scale_x_flat = features['scale_x']
        scale_y_flat = features['scale_y']
        offset_x_flat = features['offset_x']
        offset_y_flat = features['offset_y']
        ...
        disp_field_maps_shape = tf.stack([tile_res, tile_res, 2])
        disp_polygon_maps_shape = tf.stack([tile_res, tile_res, 3])
        image = tf.reshape(image_flat, image_shape)
        gt_polygon_map = tf.reshape(gt_polygon_map_flat, gt_polygon_map_shape)
        disp_field_map = tf.reshape(disp_field_map_flat, disp_field_maps_shape)
        disp_polygon_map = tf.reshape(disp_polygon_map_flat, disp_polygon_maps_shape)
        ...
        rotate_theta = tf.expand_dims(rotate_theta_flat,0)
        scale_x = tf.expand_dims(scale_x_flat,0)
        scale_y = tf.expand_dims(scale_y_flat,0)
        offset_x = tf.expand_dims(offset_x_flat,0)
        offset_y = tf.expand_dims(offset_y_flat,0)
        if not train:
        ...
        if train:
            ...
            rotate_theta, \
            scale_x, scale_y,\
            offset_x, offset_y = tf.cond(include_patch,
                                       lambda: [image, gt_polygon_map,
                                                disp_field_map, disp_polygon_map,rotate_theta,scale_x,scale_y,offset_x,offset_y],
                                       lambda: [tf.gather(image, empty),
                                                tf.gather(gt_polygon_map, empty),
                                                tf.gather(disp_field_map, empty),
                                                tf.gather(disp_polygon_map, empty),
                                                tf.gather(rotate_theta,empty),
                                                tf.gather(scale_x,empty),
                                                tf.gather(scale_y,empty),
                                                tf.gather(offset_x,empty),
                                                tf.gather(offset_y,empty)
                                                ])
        else:
            image, \
            gt_polygons, \
            ...
            offset_x, offset_y = tf.cond(include_patch,
                                       lambda: [image, gt_polygons, disp_polygons, gt_polygon_map, disp_field_map,
                                                disp_polygon_map,rotate_theta,scale_x,scale_y,offset_x,offset_y],
                                       lambda: [
        ...
        if train:
            image_batch, gt_polygon_map_batch, disp_field_map_batch, disp_polygon_map_batch,rotate_theta,scale_x,scale_y,offset_x,offset_y = tf.train.batch(
                [image, gt_polygon_map, disp_field_map, disp_polygon_map,rotate_theta,scale_x,scale_y,offset_x,offset_y],
                batch_size=batch_size,
                num_threads=8,
                dynamic_pad=True,
                enqueue_many=True,
                allow_smaller_final_batch=False)
            return image_batch, None, None, gt_polygon_map_batch, disp_field_map_batch, disp_polygon_map_batch,rotate_theta,scale_x,scale_y,offset_x,offset_y
            # image_batch, gt_polygon_map_batch, disp_field_map_batch, disp_polygon_map_batch,rotate_theta,scale_x,scale_y,offset_x,offset_y = tf.train.shuffle_batch(
            #     [image, gt_polygon_map, disp_field_map, disp_polygon_map,rotate_theta,scale_x,scale_y,offset_x,offset_y],
            #     batch_size=batch_size,
            ...
            image_batch, gt_polygons_batch, disp_polygons_batch, gt_polygon_map_batch, disp_field_map_batch, disp_polygon_map_batch,rotate_theta,scale_x,scale_y,offset_x,offset_y= tf.train.batch(
                [image, gt_polygons, disp_polygons, gt_polygon_map, disp_field_map, disp_polygon_map,rotate_theta,scale_x,scale_y,offset_x,offset_y],
                batch_size=batch_size,
                num_threads=8,
                dynamic_pad=True,
                enqueue_many=True,
                allow_smaller_final_batch=False)
            return image_batch, gt_polygons_batch, disp_polygons_batch, gt_polygon_map_batch, disp_field_map_batch, disp_polygon_map_batch,rotate_theta,scale_x,scale_y,offset_x,offset_y
...
def main():
    ...
    # ])
    data_dir = python_utils.choose_first_existing_path([
        # "/home/elsie/PycharmProjects/mapalignment-master/data"
        "/home/elsie/PycharmProjects/displacement/data"
    ])

    tfrecords_dir_list = [
        # os.path.join(data_dir, "AerialImageDataset/tfrecords.mapalign.multires"),
        # os.path.join(data_dir, "bradbury_buildings_roads_height_dataset/tfrecords.mapalign.multires.patch"),
        os.path.join(data_dir, "bradbury_buildings_roads_height_dataset/classification"),
    ...
    # queue.
    image, gt_polygons, disp_polygons, gt_polygon_map, disp_field_map, disp_polygon_map,rotate_theta,scale_x,scale_y,offset_x,offset_y = read_and_decode(
        dataset_filename_list,
        patch_inner_res,
        patch_outer_res,
        batch_size,
    ...
    with tf.Session() as sess:
        ...
        for i in range(100):
            ...
            if train:
                ...
                image_batch, gt_polygon_map_batch, disp_field_map_batch, disp_polygon_map_batch,\
                rotate_theta_batch,scale_x_batch,scale_y_batch,offset_x_batch,offset_y_batch = sess.run(
                    [image, gt_polygon_map, disp_field_map, disp_polygon_map,rotate_theta,scale_x,scale_y,offset_x,offset_y])
                print(rotate_theta_batch,scale_x_batch,scale_y_batch,offset_x_batch,offset_y_batch)
                print("end")
            ...
            skimage.io.imsave("gt_polygon_raster.png", gt_polygon_map_batch[0])
